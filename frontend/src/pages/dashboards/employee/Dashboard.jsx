...
export default function Dashboard() {
  ...
  const { data: pendingCount } = useFetch(branchId ? `/orders/count?status=PENDING` : null, [branchId]);
  const { data: totalCount } = useFetch(branchId ? `/orders/count` : null, [branchId]);
  const { data: todayAppts } = useFetch(branchId ? `/appointments/today?branchId=${branchId}` : null, [branchId]);
  const { data: alerts } = useFetch(branchId ? `/inventory/alerts?branchId=${branchId}` : null, [branchId]);
  const { data: recent } = useFetch(branchId ? `/orders?limit=6&page=1` : null, [branchId]);
