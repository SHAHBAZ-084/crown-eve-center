...
const LoginPage = ({ onLogin }) => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.user?.role !== "BRANCH_OWNER") {
        setError("Access restricted to Branch Owner accounts.");
        setL(false); return;
      }
    ...
    <div className="login-wrap">
      <div className="login-card">
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 52, height: 52, background: "var(--acc)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-d)", fontSize: 24, color: "#fff", margin: "0 auto 14px" }}>CE</div>
          <div style={{ fontFamily: "var(--font-d)", fontSize: 32, letterSpacing: ".04em" }}>BRANCH PORTAL</div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>BRANCH_OWNER access only</div>
        </div>
        {error && <div className="login-err">{error}</div>}
        <form onSubmit={submit}>
          <div className="fg"><label>Email</label><input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="branch@crowneve.com" autoFocus /></div>
          <div className="fg"><label>Password</label><input type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" /></div>
          <button type="submit" className="btn btn-p" style={{ width: "100%", justifyContent: "center", marginTop: 8 }} disabled={loading}>
            {loading ? "Authenticating…" : "Sign In to Branch Portal"}
          </button>
        </form>
        <div style={{ marginTop: 20, padding: "12px 14px", background: "var(--surf2)", borderRadius: "var(--r-md)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--muted)", marginBottom: 4 }}>API endpoint</div>
          <div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-m)" }}>{API_BASE}</div>
        </div>
      </div>
    </div>
  );
